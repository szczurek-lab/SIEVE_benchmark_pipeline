...
if [[ ${USETASKSET} -eq 1 ]]; then
  ...
  nohup taskset -c ${STARTCPU}-${ENDCPU} snakemake -s "$1" --use-conda --resources mem_mb="${MEMORY}" --cores "${CORENUM}" "${ATTACHMENTS}" &>"${LOGFILE}" &
elif [[ ${USETASKSET} -eq 0 ]]; then
  nohup snakemake -s "$1" --use-conda --resources mem_mb="${MEMORY}" --cores "${CORENUM}" "${ATTACHMENTS}" &>"${LOGFILE}" &
else
